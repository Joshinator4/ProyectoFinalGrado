...
{
    // Verificar que la orden esté pendiente antes de eliminarla
    if ($order->status === 'pending') {
        // Restaurar el stock de cada producto en la orden
        ...
        // Eliminar la orden después de restaurar stocks
        $order->delete();

        return redirect()->route('orders.index')->with('success', 'Order cancelled and stock restored.');
    }

    return redirect()->route('orders.index')->with('error', 'Order cannot be cancelled.');
}
